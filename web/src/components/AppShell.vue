...
import { House, Library, Layers, ChartNoAxesColumn } from "@lucide/vue";
...
<template>
  <div class="flex min-h-dvh flex-col">
    ...
    >
      <ul class="mx-auto grid max-w-6xl grid-cols-4">
        <li v-for="item in items" :key="item.to">
          <RouterLink
            :to="item.to"
            class="flex flex-col items-center gap-1 py-2 text-[11px] font-medium"
            :class="active(item.name) ? 'text-amber-700 dark:text-amber-400' : 'text-stone-500 dark:text-stone-400'"
          >
